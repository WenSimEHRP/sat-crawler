#!/usr/bin/env python3
from typing import Any, Union, List, Dict, Literal, Optional, Callable
import json
import weasyprint
import argparse
...
def main(
    ...
    if progress_callback:
        progress_callback(0, "Starting PDF generation...")
    ...
    if progress_callback:
        progress_callback(5, f"Using paper size: {validated_paper_size}")

    # Load questions data
    if progress_callback:
        progress_callback(10, "Loading questions data...")
    ...
    if not answers_only:
        ...
        if progress_callback:
            progress_callback(20, "Generating questions HTML...")
    ...
    if not no_answers:
        ...
        if progress_callback:
            progress_callback(70, "Generating answer key HTML...")
        ...
        if progress_callback:
            progress_callback(85, "Writing answer key HTML file...")
        write_html_file(answers_html_content, answers_html_filename)

        if progress_callback:
            progress_callback(95, "Generating answer key PDF...")
    ...
    if progress_callback:
        progress_callback(100, "PDF generation completed successfully!")
